...
function verifyPassword(type = "default") {
    ...
    if (password_input.value.length >= 8) {
        ...
        if (type == 'default') eight_characters.style.color = 'rgba(43, 43, 43, 0.4)';
    ...
    if (has_lower_input && has_upper_input) {
        ...
        if (type == 'default') has_upper_lower_char.style.color = 'rgba(43, 43, 43, 0.4)';
        ...
        if (verifyFields([name, email, code])) {
            fetch('/usuarios/cadastrarUsuario', {
                ...
                if(response.status == 403){
                    code.insertAdjacentHTML('afterend', '<span style="color: #f00; text-align: center; font-size: 12px">Código inválido</span>')
                }else{
