...
function buscarServidorPorId(req, res) {
  ...
  servidorModel
    .buscarServidorPorId(idServidor)
    .then((resultado) => {
...
function atualizarServidor(req, res) {
  ...
  servidorModel
    .atualizarServidor(idServidor, nome, modelo, so, tipo)
    .then((resultado) => res.json(resultado))
    .catch((erro) => {
      console.log(erro);
      res.status(500).json(erro.sqlMessage);
...
function buscarParametros(req, res) {
  ...
  servidorModel
    .buscarParametrosPorIdServidor(idServidor)
    .then((resultado) => {
      if (resultado.length > 0) {
        res.status(200).json(resultado);
      } else {
...
function atualizarParametros(req, res) {
  ...
  servidorModel
    .atualizarParametros(idServidor, dados)
    .then((resultado) => {
      res.json(resultado);
    })
    .catch((erro) => {
...
function obterServidoresCodecs(req, res) {
  const idEmpresa = req.params.idEmpresa;
  servidorModel
    .obterServidoresCodecs(idEmpresa)
    .then((resultado) => res.json(resultado))
    .catch((erro) => {
      console.log(erro);
      res.status(500).json(erro.sqlMessage);
