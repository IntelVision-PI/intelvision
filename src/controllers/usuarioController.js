...
function atualizarCadastro(req, res) {
  ...
  usuarioModel
    .atualizarCadastro(id, nome, email, senha)
...
function excluirUsuario(req, res) {
  ...
  if (id == undefined) {
    ...
    usuarioModel
      .excluirUsuario(id)
      .then(function (resultadoExcluir) {
      ...
      .catch(function (erro) {
        console.log(erro);
        console.log(
          "\nHouve um erro ao excluir o usuário! Erro: ",
          erro.sqlMessage
        );
        res.status(500).json(erro.sqlMessage);
      });
...
function retornaTodosOsUsuariosDaEmpresa(req, res) {
  ...
  usuarioModel
    .retornaTodosOsUsuariosDaEmpresa(id, fkEmpresa)
    .then(function (resultado) {
...
function atualizaSenhaDoUsuario(req, res) {
  ...
  usuarioService
    .atualizaSenhaDoUsuario(id, senhaAtual, senhaNova)
    .then(function (resultado) {
...
function atualizaEmailDoUsuario(req, res) {
  ...
  usuarioService
    .atualizaEmailDoUsuario(id, email)
    .then(function (resultado) {
      if (resultado.affectedRows == 0) {
        return res.json({ message: "usuário não encontrada" });
      }
...
function alterarAtividade(req, res) {
  const id = Number(req.params.idUsuario);
  const novoStatus = Number(req.body.statusUser);

  usuarioModel
    .alterarAtividade(id, novoStatus)
    .then(function (resultado) {
      if (resultado.affectedRows == 0) {
        return res.json({ message: "usuário não encontrada" });
      }
