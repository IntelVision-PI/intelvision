#!/bin/bash
  ...
  KEY_NAME="servicekey_lucas3"
  SECURITY_GROUP_NAME="intelvision-sg"
...
echo "* Inicializando Devops IntelVision *"
...
then
  echo "Criando grupo de segurança: '$SECURITY_GROUP_NAME'"
  ...
  echo "Usando grupo de segurança existente: $SG_ID"
fi
...
do
  BUCKET_NAME="$s3-$(date +%Y%m%d%H%M%S)-$RANDOM"
...
echo "* Devops IntelVision finalizado *"
